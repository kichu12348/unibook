...
const ManageStaffScreen: React.FC = () => {
  ...
  const { teachers, isSearchingTeachers, searchTeachers, requestStaff, isSubmitting } = useForumHeadStore();
  ...
  const handleRequest = (teacher: Teacher) => {
    Alert.alert(
      "Confirm Request",
      `Are you sure you want to request ${teacher.fullName} to be a staff member for "${eventName}"?`,
      [
        { text: "Cancel", style: "cancel" },
        { text: "Send Request", onPress: () => requestStaff(eventId, teacher.id) },
      ]
    );
  };
  ...
  const TeacherCard = ({ item }: { item: Teacher }) => (
    <View style={styles.card}>
      ...
      <StyledButton title="Request" size="small" onPress={() => handleRequest(item)} loading={isSubmitting} />
    </View>
  );

  const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: theme.colors.background },
    header: { paddingHorizontal: 24, paddingTop: insets.top + 16, paddingBottom: 16 },
    ...
    card: { backgroundColor: theme.colors.surface, borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: theme.colors.border, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle} numberOfLines={1}>For: {eventName}</Text>
      </View>
      ...
      <View style={styles.searchContainer}>
        <StyledTextInput
          ...
          leftElement={<Ionicons name="search" size={20} color={theme.colors.textSecondary} />}
        />
      </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>
                {debouncedSearchTerm ? `No teachers found for "${debouncedSearchTerm}"` : "No teachers available."}
              </Text>
            </View>
          }
        />
